...
public class DeliveryXactHandler_join extends XactHandler {
	// inputs
	private int W_ID;
	...
	public DeliveryXactHandler_join(Connection conn, int w_id, int carrier_id) {
	...
	void process() throws SQLException {
		...
		String sql_update_ol = String.format(
			...
			"		from orders join (select ol_w_id, ol_d_id, ol_o_id from order_line where ol_w_id = %d) ol\n" +
		...
		String sql_update_customer = String.format(
			...
			"		group by o_w_id, o_d_id, o_carrier_id),\n" +
			"item_amount as \n" +
			"	(select o_w_id, o_d_id, o_id, sum(ol_amount) as sum_amount\n" +
			"		from orders join\n" + 
